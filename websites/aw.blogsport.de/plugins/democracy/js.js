...
var SendDemocracy = "plugins/democracy/democracyf1b6.html?demSend=true";	/* modified by gigi */
var GetDemocracy = "plugins/democracy/democracyd060.html?demGet=true";
...
function SeeResults() {
	...
	if (httpReceiveVotes.readyState == 4 || httpReceiveVotes.readyState == 0) {
  		httpReceiveVotes.open("GET/index.html",GetDemocracy + '&poll_id='+poll_id+'&rand='+Math.floor(Math.random() * 1000000), true);
...
function SendVote (the_vote) {
    ...
    if (cookie) {
		...
		if (httpSendVotes.readyState == 4 || httpSendVotes.readyState == 0) {
			httpSendVotes.open("POST/index.html", SendDemocracy, true);
